...
export default function EditDeck({ deck, collection }) {
    // Initialize selected cards from the deck data
    const initialSelectedCards = deck.deck_cards.map((deckCard) => ({
        id: deckCard.id,
        pokemon_card: deckCard.pokemon_card,
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(route("pokemon.decks.update", deck.id));
    };

    return (
        <MainLayout>
            <Head title={`Edit Deck: ${deck.name}`} />
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="bg-surface overflow-hidden shadow-sm sm:rounded-lg p-4 sm:p-6">
                        <div className="flex items-center mb-6">
                            <Link
                                href={route("pokemon.decks.show", deck.id)}
